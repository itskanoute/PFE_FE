...
import { getResponsableProfile, updateResponsableProfile, updateResponsablePassword } from '../../services/api';
...
const ResponsableProfil = () => {
  ...
  const loadProfile = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await getResponsableProfile();
      ...
      setProfileData({
        ...
        email: p.email || '',
        phone: p.phone || '',
        role: p.role || 'Responsable pédagogique',
        initials: p.initials || '',
      });
    } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    setSaveError('');
    try {
      await updateResponsableProfile({
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    setPasswordError('');
    try {
      await updateResponsablePassword({
  ...
  const initials = profileData.initials || `${profileData.firstName?.[0] || ''}${profileData.lastName?.[0] || ''}`.toUpperCase();
        ...
        <div style={{ flex: '1 1 400px', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <div style={{ position: 'relative', marginBottom: '1rem' }}>
              ...
              <button onClick={() => document.getElementById('avatar-upload').click()} style={{ position: 'absolute', bottom: 0, right: 0, backgroundColor: '#0f172a', color: 'white', width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '2px solid white' }}>
                <Camera size={16} />
              </button>
            </div>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '1.5rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <button onClick={() => { setPasswordError(''); setIsPasswordModalOpen(true); }} style={{ backgroundColor: 'transparent', border: '1px solid #cbd5e1', color: '#0f172a', padding: '0.8rem', borderRadius: '8px', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', textAlign: 'center' }}>
        ...
        <div style={{ flex: '2 1 600px', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <form onSubmit={handleSave}>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input
                      ...
                      readOnly
                      style={{ width: '100%', padding: '0.75rem 1rem 0.75rem 2.5rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '0.95rem', color: '#64748b', backgroundColor: '#f8fafc' }}
                    />
                  </div>
                </div>
                <div>
              ...
              <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', border: '1px solid #f1f5f9', borderRadius: '8px' }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                    <div style={{ position: 'relative' }}>
                      <input type="checkbox" checked={notifications.emailNouvelleCandidature} onChange={() => handleToggle('emailNouvelleCandidature')} style={{ opacity: 0, width: 0, height: 0 }} />
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', border: '1px solid #f1f5f9', borderRadius: '8px' }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                    <div style={{ position: 'relative' }}>
                      <input type="checkbox" checked={notifications.emailHeuresAValider} onChange={() => handleToggle('emailHeuresAValider')} style={{ opacity: 0, width: 0, height: 0 }} />
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', border: '1px solid #f1f5f9', borderRadius: '8px' }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                    <div style={{ position: 'relative' }}>
                      <input type="checkbox" checked={notifications.smsUrgence} onChange={() => handleToggle('smsUrgence')} style={{ opacity: 0, width: 0, height: 0 }} />
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '1.5rem' }}>
              {saveError && (
                <div style={{ color: '#dc2626', fontWeight: 600, fontSize: '0.9rem' }}>{saveError}</div>
              )}
              {isSaved && (
              ...
              <button type="submit" disabled={saving} style={{ backgroundColor: '#0f172a', color: 'white', border: 'none', padding: '0.8rem 2rem', borderRadius: '8px', fontWeight: 700, fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', opacity: saving ? 0.6 : 1 }}>
                <Save size={20} />
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '12px', width: '100%', maxWidth: '400px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            ...
            <form onSubmit={handlePasswordSubmit} style={{ padding: '1.5rem' }}>
              ...
              {passwordError && (
                <div style={{ color: '#dc2626', fontSize: '0.85rem', marginBottom: '1rem' }}>{passwordError}</div>
              )}
              ...
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                ...
                <button type="submit" disabled={passwordSaving} style={{ backgroundColor: '#0f172a', color: 'white', border: 'none', padding: '0.6rem 1.5rem', borderRadius: '6px', fontWeight: 600, cursor: 'pointer', opacity: passwordSaving ? 0.6 : 1 }}>
